...
std::string var_expression_t::generate_code(std::ostream& os, code_gen::CodeGen_Context& ctx, int32_t flags) const
{
	...
	return std::to_string(actual_offset(ctx.frame_size,var_info.offset));

}

std::string func_decl_t::generate_code(std::ostream& os, code_gen::CodeGen_Context& ctx,int32_t flags) const
{
...
std::string integer_literal_t::generate_code(std::ostream& os, code_gen::CodeGen_Context& ctx, int32_t flags) const
{
	if(value == 0){
		return "zero";
	}
