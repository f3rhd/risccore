...
namespace f3_riscv_assembler {
	...
	public:
		...
		void parse_lines(std::vector<Line>& lines,std::unordered_map<std::string,uint32_t>& labels);

	private:
		void resolve_label_identifier(Ast_Node* head,std::unordered_map<std::string,uint32_t>& labels);
