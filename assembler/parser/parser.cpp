...
void Parser::parse_lines(std::vector<Line> &lines, std::unordered_map<std::string, uint64_t> &labels) {
...
void Parser::resolve_label_identifier(Ast_Node *head, std::unordered_map<std::string, uint64_t> &labels) {
    ...
    if (candidate_label_identifier_node && candidate_label_identifier_node->node_type == AST_NODE_TYPE::IDENTIFIER) {
        ...
        uint64_t label_row_number = it->second;
